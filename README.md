...
- `-` 去掉了语音相关功能，现在已有更好用的 AI 方式

- `-` 去掉了对 LOL 的支持，我自己不玩无法验证其可行性，而且现在也有更好的支持方式
...
本项目支持用户自定义词典功能，用于指定专有名词的翻译方式。

编辑与主程序同目录下的 `dictionary.md` 文件，按模板格式添加词条即可。
程序会自动读取词典，LLM 在翻译时会优先使用词典中的翻译。
...
| `/persona <个性描述>` | 切换翻译的个性，有助于特定领域的用词准确性 | `/persona 电影导演 ` 、`/persona 程序员`|
...
此功能已移除并归档到 `archive/lol_feature/` 目录。
如需恢复，请参考 `archive/lol_feature/RESTORE_GUIDE.md`。
